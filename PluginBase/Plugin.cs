...
namespace MigrationHelper.PluginBase
{
    ...
    public abstract class Plugin : IPlugin
    {
        ...
        public Plugin(Type pluginClassName)
        {
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            {
                localPluginContext.Trace($"Exception: {exception.ToString()}");
            ...
            catch (Exception exception)
            {
                ...
                throw new InvalidPluginExecutionException($"Exception: {exception.Message}", exception);
            }
            finally
            {
    ...
    public interface ILocalPluginContext
    {
        ...
        T GetTargetEntity<T>(bool doThrowWhenMissing = true) where T : Entity;
        T GetPostImage<T>(bool doThrowWhenMissing = false) where T : Entity;

        EntityReference GetTargetEntityReference(bool doThrowWhenMissing = true);
        T GetPreImage<T>(bool doThrowWhenMissing = true) where T : Entity;
    }
    ...
    public class LocalPluginContext : ILocalPluginContext
    {
        ...
        public T GetTargetEntity<T>(bool doThrowWhenMissing = true) where T : Entity
        {
            T targetEntity = null;
            if (PluginExecutionContext.InputParameters.Contains(BaseWellKnowns.InputParameter_Target) &&
                PluginExecutionContext.InputParameters[BaseWellKnowns.InputParameter_Target] is Entity)
            {
                var entity = (Entity)PluginExecutionContext.InputParameters[BaseWellKnowns.InputParameter_Target];
                targetEntity = entity.ToEntity<T>();
                Trace($"casted target of type {entity.LogicalName}");
            }
            else
            {
                if(doThrowWhenMissing)
        ...
        public EntityReference GetTargetEntityReference(bool doThrowWhenMissing = true)
        {
            EntityReference targetEntity = null;
            if (PluginExecutionContext.InputParameters.Contains(BaseWellKnowns.InputParameter_Target) &&
                PluginExecutionContext.InputParameters[BaseWellKnowns.InputParameter_Target] is EntityReference)
            {
                targetEntity = (EntityReference)PluginExecutionContext.InputParameters[BaseWellKnowns.InputParameter_Target];
            }
            else
            {
                if(doThrowWhenMissing)
                    throw new InvalidPluginExecutionException($"Target reference is missing -> {PluginExecutionContext.InputParameters[BaseWellKnowns.InputParameter_Target]}");
            }

            return targetEntity;
        }

        public T GetPostImage<T>(bool doThrowWhenMissing = true) where T : Entity
        {
            T postImage = default;
            if (PluginExecutionContext.PostEntityImages.ContainsKey(BaseWellKnowns.EntityImage_PostImage))
            {
                Trace($"casting post image");
                postImage = PluginExecutionContext.PostEntityImages[BaseWellKnowns.EntityImage_PostImage].ToEntity<T>();
                Trace($"casted post image");
            }
            else
            {
                if (doThrowWhenMissing)
                    throw new InvalidPluginExecutionException("PostImage is missing");
            }

            return postImage;
        }

        public T GetPreImage<T>(bool doThrowWhenMissing = true) where T : Entity
        {
            T preImage = default;
            if (PluginExecutionContext.PreEntityImages.ContainsKey(BaseWellKnowns.EntityImage_PreImage))
            {
                Trace($"casting pre image");
                preImage = PluginExecutionContext.PreEntityImages[BaseWellKnowns.EntityImage_PreImage].ToEntity<T>();
                Trace($"casting pre image");
            }
            else
            {
                if (doThrowWhenMissing)
                    throw new InvalidPluginExecutionException("PreImage is missing");
            }
